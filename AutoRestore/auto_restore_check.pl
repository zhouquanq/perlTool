		...
		foreach my $section (@{$cfg_ini}) {
			...
			if ($dbname_base ne "restore" && $dbname_base ne "ftp" ) {
				...
				foreach (@{$run_servers}) {
						...
						foreach (@items) {
							next unless /(\d+)_full\.tar\.gz$/;
							$newest_full_time = $1 if $1 > $newest_full_time;
						}
						foreach (@items) {
							next unless /((\d+)_(full|inc)\.tar\.gz)$/;
						...
						while ($flag) {
							...
							ftp_down_file6($ftp, $ftp_from_dir, $downpath, $b_files);
						...
						if(!defined($restore_return)) {
							...
							log2("starttime:".$ftp_start_time."  lasttime:".ts2str($last_ts));
							if(str2ts($ftp_start_time) - $last_ts > 7200) {
